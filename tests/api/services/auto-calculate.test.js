...
describe('auto-calculate', () => {
  ...
  it('sets DESS to Node-RED when starting with writeToVictron and dessModeEntity', async () => {
    ...
    const settings = makeSettings({
      ...
      updateData: false,
      writeToVictron: true,
      dessModeEntity: 'select.dess_mode',
    });
    // haToken required so setDessMode doesn't bail early
    settings.haToken = 'test-token';
    settings.haUrl = 'http://homeassistant.local';
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      expect.stringContaining('/api/services/select/select_option'),
      expect.objectContaining({
        method: 'POST',
        body: expect.stringContaining('"option":"Node-RED"'),
      }),
    );
  ...
  it('handles setDessMode failure gracefully', async () => {
    // Stub fetch to reject for DESS mode call
    const dessFailFetch = vi.fn().mockRejectedValue(new Error('DESS API down'));
    vi.stubGlobal('fetch', dessFailFetch);
    ...
    // Should not throw even though DESS mode API fails
    startAutoCalculate(settings);

    // Wait for async setDessMode to complete
    await vi.advanceTimersByTimeAsync(0);

    expect(warnSpy).toHaveBeenCalledWith(
      expect.stringContaining('[auto-calculate] Failed to set DESS mode'),
      expect.any(String),
    );
  ...
  it('reverts DESS to Auto / VRM on stop', async () => {
    const fetchMock = vi.fn().mockResolvedValue({ ok: true });
    vi.stubGlobal('fetch', fetchMock);
    ...
    // Reset call count so we only observe the stop-triggered fetch
    fetchMock.mockClear();
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        method: 'POST',
        body: expect.stringContaining('"option":"Auto / VRM"'),
      }),
    );
